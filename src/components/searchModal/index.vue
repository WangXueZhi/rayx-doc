      ...
      <r-scroll-bar class="search-scroll-wrapper">
        <div>
          <div v-for="(item, index) in state.searchRes" :key="index">
            <div
              v-if="item.isPageName"
              class="search-scroll-item"
              @click="goPage(item.data.url.slice(1))"
            >
              <r-icon
                name="iconform"
                :size="20"
                class="search-scroll-item-left"
              />
              <div
                v-html="item.matchHtml"
                class="search-scroll-item-content search-scroll-item-mainText"
              ></div>
            </div>
            <div
              v-else
            ...
            >
              ...
              <div class="search-scroll-item-content">
                <div
                  v-html="item.matchHtml"
                  class="search-scroll-item-mainText"
                ></div>
                <div class="search-scroll-item-content-heading">
                  {{ item.data.name }}
                </div>
              </div>
            </div>
        ...
        store.state.docsData.forEach((item: any) => {
          if (item.name.includes(v)) {
            state.searchRes.push({
              ...
              isPageName: true,
              pageName: item.name,
              matchHtml: `<div>${item.name.replace(
                v,
                `<span style="color: var(--main-color)">${v}</span>`
              )}</div>`,
              searchValue: v,
            })
          }
